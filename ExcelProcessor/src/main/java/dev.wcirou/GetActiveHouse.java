...
public class GetActiveHouse implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    //Getting Logger
    private static final Logger logger = LoggerFactory.getLogger(GetActiveHouse.class);
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        try {
            ...
            if (input.getQueryStringParameters().get("Address") == null) {
                item = utils.activeQueryByMLSNum(ddb, input.getQueryStringParameters().get("MLSNum"));
            } else if (input.getQueryStringParameters().get("MLSNum") == null) {
                item = utils.activeQueryByAddress(ddb, input.getQueryStringParameters().get("Address"));
            } else {
                item = utils.activeQueryByMLSNum(ddb, input.getQueryStringParameters().get("MLSNum"));
            }
